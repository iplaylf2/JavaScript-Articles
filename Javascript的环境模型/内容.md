...
函数f2保存了环境\$f1的引用，环境\$1保存了全局环境的引用。这种信息同样可以从[[Scopes]]得到。  
还能从“Closure (f1)”得知环境\$f1包含了绑定\*s1。Closure相当于作用域的类型，除了全局作用域是Global，其他作用域都属于Closure。
...
与前文相比，只是少掉一行`return s1;`，“Closure (f1)”、环境\$f1却不见了。  
实际上，这是环境模型的实践被JS优化过所造成的结果。  
解释器在执行代码之前会对代码进行分析。从分析中，可以知道一些变量除了定义它的函数，**永远**不会被别的函数使用。这意味着，这些变量可以在函数返回时从当前环境中移除，而不影响到后续代码的运行。  
甚至，如果设置一种专门用来被别的环境引用的环境，那么只有那些可能被其他函数所用到的变量，才会加入到这种环境中。那些永远不会被别的函数使用的变量，就能进一步地，在函数不需要他们时提前被释放。  
JS就是如此，作用域只会捕捉那些有可能被其他函数使用的变量。进一步地，如果一个函数没有定义变量，亦或是它的变量都不被其他函数所用，那么
