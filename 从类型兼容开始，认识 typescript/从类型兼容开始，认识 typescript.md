...
理解类型兼容是理解 `as` 、`extends` 、`infer` 等符号的开始。
...
JavaScript 广泛使用了函数表达式和对象字面量，结构化类型就是针对这点设计的，他们在字面表达上有非常相似的地方。而结构化类型的字面表达，与泛型类型、条件类型，类型推导等类型操作关系密切。
...
- 函数参数中，`x` 的类型 `unknown` 向着子类型 `number` 的方向，也就是向上转换，表现出了逆变。
- 函数返回类型 `number` 向着超类型 `unknown` 的方向，也就是向下转换，表现出了协变。
...
- `In` 泛型参数表现出了从 `unknown` 到 `number` 的逆变。
- `Out` 泛型参数表现出了从 `number` 到 `unknown` 的协变。 

函数类型向下兼容的表现是，当类型不同时，参数列表上的类型参数能够逆变、返回值的类型参数能够协变。

### 为什么会表现出逆变和协变

为什么函数类型向下兼容时，不同部位的类型分别会表现出逆变和协变呢？

**因为类型分配的方向始终需要向下兼容**。
- 函数参数类型的向下兼容路径，是参数输入的路径，方向与整个函数类型的向下兼容路径相反。因此参数类型表现出的是逆变。
- 而返回类型的向下兼容路径，是返回输出的路径，方向与整个函数类型的向下兼容路径一致。因此返回类型表现出的是协变。

如下图例子所示：
![img](./9.png)
- 参数输入的路径可以看作是，`123` → `x: number` → `x: unknown` ，类型在路径上向下兼容。但与整个函数类型的向下兼容路径 `foo` ← `bar` 的方向相反。因此参数类型表现出了逆变。
- 返回输出的路径可以看作是, `number` → `unknown` → `unknown` ，类型在路径上向下兼容。与整个函数类型的向下兼容路径的方向一致。因此返回类型表现出了协变。

### 双变带来了陷阱

下面是一段编译时不会报错的代码，但它在运行时却会因为类型不兼容而出错。
```typescript
type Troy<T> = {
  hide(x: T): void;
};

const foo: Troy<string> = { hide: (x) => x.split(",") };
const bar: Troy<unknown> = foo;

bar.hide(233); // x.split is not a function 
```
- `Troy<T>` 不会根据上下文约束泛型参数 `T` 所需要的逆变，因此产生了类型兼容上的陷阱。

实际上， `Troy<T>` 不但不会根据上下文自动约束 `T`，甚至连直观的协变也不约束。
```typescript
type Troy<T> = {
  hide(x: T): void;
};

declare let foo: Troy<string>;
declare let bar: Troy<unknown>;

foo = bar;
bar = foo;

// 那 Troy<T> 对 T 就没有任何约束么？

const baz: Troy<number> = foo; // 有的，不能兼容，编译报错
```
上述例子会给人带来两个疑问：
1. `Troy<string>` 和 `Troy<unknown>` 互相向下兼容？他们类型相等？
2. 为什么 `Troy<string>` 可以兼容 `Troy<unknown>` ，却不能兼容 `Troy<number>` 。

其实这都是 TypeScript 不严格的泛型类型带来的问题。一般情况下泛型类型的类型参数是支持双变的。所谓双变，就是允许类型参数同时支持逆变和协变。只要类型参数满足逆变和协变的其中一条转换，就能使泛型类型兼容，类似于 `as` 。

上述疑问的解答就是：
1. `Troy<string>` 和 `Troy<unknown>` 只是在不严格条件下类型互相兼容，他们类型不相等。
2. `string` 和 `unknown` 存在包含关系，因此 `Troy<string>` 和 `Troy<unknown>` 在不严格条件下可以互相兼容。而 `string` 和 `number` 不存在包含关系，因此 `Troy<string>` 在不严格条件下也不能兼容 `Troy<number>` 。

泛型类型的双变也许方便了我们编程使用，但不严格的特性同时会给我们带来隐藏的陷阱。
...
因为条件类型的“分布律”呀。如果条件类型呈现“分布律”就会对“中间结果”做联合运算（ `|` ），而任何类型与 `never` 联合，如同任何集合与空集合并，得到的是该类型本身。这样一来，条件类型呈现“分布律”时，得到的“中间结果”为 `never` 的项将会被“忽略”。如果不是刻意避免，条件类型很容易就带有“分布律”性质，此时对无用的分支返回 `never` 是一个不容易出错且合乎直观的选择。
