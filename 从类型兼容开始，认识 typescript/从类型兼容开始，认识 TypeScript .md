...
因为条件类型的分布律呀。如果条件类型呈现分布律就会对“中间结果”做联合运算（ `|` ），而任何类型与 `never` 联合，如同任何集合与空集合并，得到的是该类型本身。这样一来，条件类型呈现分布律时，得到的“中间结果”为 `never` 的项将会被“忽略”。

如果不是刻意避免，条件类型很容易就带有分布律性质，此时对“无用”的分支返回 `never` 是一个不容易出错且合乎直观的选择。
